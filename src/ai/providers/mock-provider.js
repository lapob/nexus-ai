/**
 * @module ai/providers/mock-provider
...
class MockProvider {
  ...
  async createModel(specification) {
    const model = {
      ...
      provider: 'mock',
      size: 1,
      capabilities: { chat: true, streaming: true, embeddings: false, tools: false, vision: false, thinking: false }
    };
    this.models.push(model);
    this.model = model.id;
    return model;
  }
  async preloadModel(model) { return { status: 'ready', model: model || this.model }; }
  async pullModel(model, { onProgress } = {}) { onProgress?.({ status: 'success', model, completed: 1, total: 1, percent: 100 }); return { status: 'success', model }; }
