...
function createCosmicVisualizers(canvas, options = {}, createRecipes) {
  ...
  let suppressClick = 0, paintedReduced = false, ringVisibility = 0;
  ...
  const prefix = `precision highp float;attribute vec3 position;attribute vec3 aJourney;uniform mat4 modelViewMatrix;uniform mat4 projectionMatrix;uniform vec4 uStage;uniform float uArrival;uniform float uDpr;varying float vJourney;varying float vEdge;`;
  function vertex(source) {
    ...
    return prefix + source.slice(0,index) + `
      ...
      vJourney=settle;
      vEdge=smoothstep(0.,.08,1.-abs(gl_Position.x))*smoothstep(0.,.08,1.-abs(gl_Position.y));
    }`;
  }
  function fragment(source) {
    const index=source.lastIndexOf('}');
    // Sparse distant grains become the dense desktop matter, without a flashed layer.
    return 'precision highp float;varying float vJourney;varying float vEdge;'+source.slice(0,index)+`gl_FragColor.a*=mix(.035,1.,vJourney)*vEdge;}`;
  }
  ...
  function draw(now){
    ...
    const coreVisible=options.getVisible?.()!==false;
    ...
    if(strain>1.5){quality=Math.max(.35,quality-.15);strain=0;}if(healthy>10){quality=Math.min(1,quality+.05);healthy=0;}
    if(ambientPaint){
      ...
      for(let i=0;i<(efficient?140:280);i++){const x=((i+901)*.61803398875%1)*width+Math.sin(t*.12+i)*3,y=((i+901)*.41421356237%1)*height+Math.cos(t*.1+i)*3;const edge=Math.min(1,Math.abs(x-width/2)/(width*.45));ambientPaint.fillStyle=`rgba(166,224,234,${.04+edge*edge*.36})`;ambientPaint.fillRect(x,y,.7+i%3*.3,.7+i%3*.3);}
    }
    if(gl){
      ...
      ringVisibility+=(ringTarget-ringVisibility)*(1-Math.exp(-dt*1.65));
      for(const [name,value] of Object.entries({projectionMatrix:projection,uStage:stage,uArrival:arrival,uDpr:dpr,uTime:t,uAudio:[audio,audio*.65,audio*.45,audio*.25],uAccent:accent,uLuminosity:1.08,uPointScale:1.05,uStateBlend:1,uStateEnergy:profile.energy,uTransition:0,uRingVisibility:ringVisibility,uDisintegration:0,uPointer:pointer,uPointerStrength:pointerStrength}))uniform(p,name,value);
      ...
      for(const field of coreVisible?fields[preset]:[]){
        ...
        uniform(p,'modelViewMatrix',matrix(rx,rotation[1]+(saturn?.04:0),saturn&&field.kind>0?-.16:reactor?.16:0,saturn&&field.kind===0?.94:reactor?1.48:1));uniform(p,'uKind',field.kind);
      ...
      for(const field of fields[preset]){const points=field.positions,step=Math.max(3,Math.ceil(points.length/3600)*3);for(let i=0;i<points.length;i+=step){const x=rect.left+rect.width/2+points[i]*side/13.2;const y=rect.top+rect.height/2-(points[i+1]*.92-points[i+2]*.38)*side/13.2;const originX=((i+1)*.61803398875%1)*width,originY=((i+1)*.41421356237%1)*height;ctx.globalAlpha=.15+arrival*.5;ctx.fillRect(originX+(x-originX)*arrival,originY+(y-originY)*arrival,1,1);}}
      ctx.globalAlpha=1;
    }
  ...
  function click(e){if(performance.now()<suppressClick){e.preventDefault();e.stopImmediatePropagation();}}
  ...
  host.addEventListener('pointermove',move,{passive:true});host.addEventListener('pointerdown',down);host.addEventListener('pointerup',leave);host.addEventListener('pointerleave',leave);host.addEventListener('pointercancel',leave);host.addEventListener('click',click,true);
  ...
  resize();refresh();
  return {refresh,setState(value){state=value;refresh();},getMetrics:()=>({state,phase,preset,previousPreset,arrival,energy:profile.energy,audio,quality,particles:Math.floor(budget*quality),draws,drawMs,backend:gl?'webgl':'canvas',fallbackReason}),dispose(){disposed=true;cancelAnimationFrame(raf);ro.disconnect();io.disconnect();ambient?.remove();document.removeEventListener('visibilitychange',visibility);media.removeEventListener('change',refresh);globalThis.removeEventListener('resize',refresh);globalThis.removeEventListener('scroll',refresh);globalThis.removeEventListener('blur',leave);host.removeEventListener('pointermove',move);host.removeEventListener('pointerdown',down);host.removeEventListener('pointerup',leave);host.removeEventListener('pointerleave',leave);host.removeEventListener('pointercancel',leave);host.removeEventListener('click',click,true);canvas.removeEventListener('webglcontextlost',contextLost);canvas.removeEventListener('webglcontextrestored',contextRestored);if(gl){for(const b of buffers)gl.deleteBuffer(b);for(const p of Object.values(programs))gl.deleteProgram(p.p);}}};
}
