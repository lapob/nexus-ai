...
function createCosmicVisualizers(canvas, options = {}, createRecipes) {
  ...
  function upload() {
    ...
    for(const list of Object.values(fields)){let offset=0;for(const field of list){
      ...
      for(let i=0;i<count;i++){const grain=i+offset,carrier=grain<(efficient?140:360)&&grain%4===0,source=carrier?grain+901:grain+1;journey[i*4]=(source*.61803398875)%1;journey[i*4+1]=(source*.41421356237)%1;journey[i*4+2]=((grain+1)*.754877666)%1;journey[i*4+3]=carrier?grain+1:0;}
      offset+=count;
  ...
  function draw(now){
    ...
    if(coreVisible&&!wasCoreVisible&&!isReduced){age=0;arrival=0;phase='assembling';}
    wasCoreVisible=coreVisible;
