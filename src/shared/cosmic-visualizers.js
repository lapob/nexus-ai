...
function createCosmicVisualizers(canvas, options = {}, createRecipes) {
  ...
  function draw(now){
    ...
    const saturnFraming=2.65-smooth(ringVisibility/.56)*1.65;
    const fieldScale=kind=>preset==='saturn-experimental'?saturnFraming*(kind===0?.94:1):preset==='jarvis-reactor'?1.48:1;
    ...
    if(gl){
      ...
      for(const field of coreVisible?fields[preset]:[]){
        ...
        for(let i=0;i<points.length;i+=step){
          ...
          ctx.globalAlpha=(.15+arrival*.5)*(saturn&&field.kind>0?ringVisibility:1);
