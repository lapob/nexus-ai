...
test('Funnel espone il listener Remote AI ma non la Console operativa', async () => {
  ...
  try {
    ...
    assert.match(publicHtml, /attribute vec4 aJourney/);
    assert.match(publicHtml, /vCarrier=step\(\.5,aJourney\.w\)/);
    ...
    assert.match(publicHtml, /releases\/download\/v0\.3\.14-preview\.3\/NexusNXS-Android\.apk/);
    ...
    assert.match(serviceWorkerSource, /nexusnxs-ai-shell-v5/);
